...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        if expressions.first.second is not nil:
            define_arguments = expressions.first.second
        else:
            define_arguments = nil
        env.define(expressions.first.first, scheme_eval(Pair("lambda", Pair(define_arguments, expressions.second)), env))
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        return scheme_eval(expressions.first, env, True)
    elif scheme_true(scheme_eval(expressions.first, env)) == False:
        return False
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        return scheme_eval(expressions.first, env, True)
    elif scheme_true(scheme_eval(expressions.first, env)) ==  True:
        return scheme_eval(expressions.first, env, True)
    else:
...
def check_formals(formals):
    ...
    """
    # BEGIN Question 11B
    def formal_iterator(pair, list):
        if pair == nil:
            return
        elif not scheme_symbolp(pair.first) or pair.first in list:
            raise SchemeError
        list.append(pair.first)
        formal_iterator(pair.second, list)
    formal_iterator(formals, [])
    # END Question 11B

    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                if self.ant.can_contain(insect) and self.ant.container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.min_range):
            holder = holder.entrance
        while holder.entrance.name != 'Hive' and  holder.entrance != None and len(holder.bees) == 0 and x < self.max_range:
            holder = holder.entrance
            x += 1
        return random_or_none(holder.bees)
        # END Problem 3B
    ...
    def action(self, colony):
        ...
        if self.eating == 0:
            if len(self.place.bees) > 0:
                ...
                self.eating = self.time_to_digest

        else:
            if self.eating <= self.time_to_digest:
                self.eating -= 1
        # END Problem 6B
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.ant:
            ...
            getattr(self.ant, 'action')(colony)
        ...
        if not self.impostor: 
            ...
            while current_place is not None: 
                if current_place.ant is not None:
                    if current_place.ant.container and current_place.ant.ant is not None:
                        if current_place.ant.container and current_place.ant.ant is QueenAnt:
                            if current_place.ant.ant.impostor:
                                current_place.ant.ant.reduce_armor(self.armor)
                        else:
                            current_place.ant.ant.damage = 2 * type(current_place.ant.ant).damage 
                    current_place.ant.damage = 2 * type(current_place.ant).damage 
